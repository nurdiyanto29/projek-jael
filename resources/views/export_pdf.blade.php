...
<thead>
	<td>
		...
		<left>
			...
			<font size="2">Hal   : Penyampaian laporan keuangan masjid per bulan {{date("m/Y")}} </font> <i><br>
			</left>
	</td>
</thead>
</table>
	...
	<table  class='table table-striped' >
		...
		<body>
			...
			<tr>
				...
				<td>{{$p->sumber}}</td>
				<td>{{"Rp.".number_format($p->masuk,2,',','.')}}</td>
				<td>{{"Rp.".number_format($p->keluar,2,',','.')}}</td>
				<td>{{$p->kategori}}</td>
			</tr>
			@endforeach
			<tr>
				...
				<td></td>
				<td>Sub Total </td>
				<td>{{"Rp.".number_format($submasuk,2,',','.')}}</td>
				<td>{{"Rp.".number_format($subkeluar,2,',','.')}}</td>
			</tr> 
			<tr>
				<td></td>
				<td></td>
				<td></td>
				<td>Total saldo </td>
				<td><td>{{"Rp.".number_format($masuk-$keluar,2,',','.')}}</td></td>
				<td></td>
			</tr> 
		</tbody>
	</table>
